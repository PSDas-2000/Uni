...
public class PrimAlgo {
	...
	public static int getMST(int[][] edges) {
		...
		int start[] = {3};
		System.out.println(String.format("Waehle %d als Wurzel", start[0]));
	...
	// for the minimal spanning tree before. The method also adjusts the
		...
		for (int j = 0; j < check.length; j++) {
			if (check[j]) {
				...
				for (int i = 0; i < edges.length; i++) {
					if (!check[i]) {
						if ((minCost == 0 || edges[fromNode - 1][i] < minCost) && edges[fromNode - 1][i]!=0) {
...
//	private static int getMinCostElem1(int[][] edges, ArrayList<Integer> check, int[] start) {
//		int minCostElem = -1;
//		int minCost = 0;
//		int fromNode = 0;
